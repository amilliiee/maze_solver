...
class Maze:
    def __init__(self, x1, y1, num_rows, num_cols, cell_sz_x, cell_sz_y, win=None, seed=None):
        self._cells = []
        self._x1 = x1
        self._y1 = y1
        self._num_rows = num_rows
        self._num_cols = num_cols
        self._cell_size_x = cell_sz_x
        self._cell_size_y = cell_sz_y
        self._win = win
        if seed:
            random.seed(seed)
    ...
    def _break_walls_r(self, i, j):
        ...
        while True:
            ...
            # NOWHERE TO GO, BREAK OUT
            if len(next_index_list) == 0:
                self._draw_cell(i, j)
                return
            
            # RANDOM DIRECTION
            dir_index = random.randrange(len(next_index_list))
            next_index = next_index_list[dir_index]
